...
M=5.05*ff*(1-3*m/sigu); %chaboche mean stress
yield=y-lam*m;                   %our mean stress
%---------------------Plot 3 methods-----------------------------
%---------------------1 Chaboche method-----------------------------
...
Wcyc=4*(E-k)*(1+nu)*(b-1)/(E*(E+k*nu)*b*(b+1))*norm(loadtensor-(1/3*sum(diag(loadtensor)))*diag([1,1,1]),'fro').^(b+1)*yield.^(1-b) ;
while Gcyc< 1
 Gcyc = Gcyc+n0*(1-alp)*(gam + 1)*Wcyc/stepnumber/WF;
