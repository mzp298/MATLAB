...
a2=0.5;                %sensitivity of sequence effect(control alp>0)
...
NF1=((1+gam)*a1*sequence1).^-1*WF*E*(E+k*nu)*b1*(b1+1)*(n0*(4*(E-k)*(1+nu)*(b1-1)))^-1.*yield.^(b1-1).*Smax.^(-b1-1);
NF2=((1+gam)*a2*sequence2).^-1*WF*E*(E+k*nu)*b2*(b2+1)*(n0*(4*(E-k)*(1+nu)*(b2-1)))^-1.*yield.^(b2-1).*Smax.^(-b2-1);
...
% hTitle =title('S-N curve at constant amplitude stress with R=-1 in our model with different $a$');
hXLabel = xlabel('N_F');
hYLabel =ylabel('Stress');

hLegend=legend([sn1,sn2], 'a=0.1','a=0.5','Location','northeast');
...
set(gcf, 'PaperPosition', [0 0 1200 1000]); %set(gcf,'PaperPosition',[left,bottom,width,height])
 saveas(gcf,'F:\Git\Anew\figures\SNa.png');