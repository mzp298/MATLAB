...
load1=5e8;            %cyclic load
load2=2e8;            %cyclic load
stepnumber=200;        %devide one cycle in 200 parts
f=50;                            %frequency of load
steptime=1/f/stepnumber;
WF=3e4;             %dissipated energy to failure per unit volume
...
a=0.1;                  %sensitivity of sequence effect
%---------------------3 Numerical method-----------------------------
G=0;
...
sequence=((Smax(1).*yield.^-1).*(X-Smax(1)*yield.^-1).^-1).^pb;
...
normSb=sqrt(sum(Sbtensor.^2)); %sum(a) sums all the colume
    ...
    W= sum(Ws);
sequence=((Smax(n+1).*yield.^-1).*(X-Smax(n+1)*yield.^-1).^-1).^pb;
sequence(sequence<0)=0;
alp=1-a*sequence;
    G = G+n0*(1-alp)*(gam + 1)*W/WF;
...
grid minor;
set(gca ,'FontSize',35);
hTitle = title('Damage evolution in Hign-Low sequence' ,'Fontsize' ,35);
