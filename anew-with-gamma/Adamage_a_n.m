...
b=1.75;                    %weakening scales distribution exponent
WF=1.296e6;            %dissipated energy to failure per unit volume
n0=1;                   %number of initial local defects
%---------------------Verified parameters in random loading case-----------------------------
lam=0;             %hydrostatic pressure sensitivity
a=0.2;                  %sensitivity of sequence effect
...
while G<1
    ...
    G(n+1) = G(n)+n0*(1-alp(n+1))*(gam+1)*W/WF;
    D(n+1) = 1-(1-G(n+1).^(1/(1-alp(n+1)))).^(1/(gam + 1));
...
%% 
G1=plot ((1:n),G(1:n),'LineStyle', 'none','LineWidth', 2, 'Marker', '^', 'MarkerSize', 10, ...
    'MarkerEdgeColor',  'none' , 'MarkerFaceColor' ,'b');
%% 
