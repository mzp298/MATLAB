...
X2=1.15;                 %Major damage threshold (meaning S_{max} must be greater than X/2*yield to activate magnification)
...
sequence1=((Smax.*yield.^-1).*(X1-Smax.*yield.^-1).^-1).^pb;
sequence2=((Smax.*yield.^-1).*(X2-Smax.*yield.^-1).^-1).^pb;
...
% hTitle =title('S-N curve at constant amplitude stress with R=-1 in our model with different $X$');
hXLabel = xlabel('N_F');
hYLabel =ylabel('Stress');

hLegend=legend([sn1,sn2], 'X=1.065','X=1.15','Location','northeast');
...
%  saveas(gcf,'F:\Git\Anew\figures\SNX.png');