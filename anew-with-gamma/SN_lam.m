...
lam2=0.3;               %hydrostatic pressure sensitivity
...
NF1=((1+gam)*a1*sequence1).^-1*WF*E*(E+k*nu)*b1*(b1+1)*(n0*(4*(E-k)*(1+nu)*(b1-1)))^-1.*yield1.^(b1-1).*Smax.^(-b1-1);
NF2=((1+gam)*a2*sequence2).^-1*WF*E*(E+k*nu)*b2*(b2+1)*(n0*(4*(E-k)*(1+nu)*(b2-1)))^-1.*yield2.^(b2-1).*Smax.^(-b2-1);
...
hLegend=legend([sn1,sn2], '\lambda=0.1','\lambda=0.3','Location','northeast');
