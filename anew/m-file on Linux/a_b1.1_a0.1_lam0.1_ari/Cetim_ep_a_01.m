...
fid = fopen('/home/ma/MATLAB/Cetim/ep_a_01/Acqui_CV.txt');
area=2.29*9.99*1e-6; %meter square
NF=99892; %number of points
...
nF=NF_num*2
xlwrite('a_fitting.xls',nF,1,'D02');
xlwrite('a_fitting.xls',(NF-nF)/NF,1,'E02');
% Smax_ben=sqrt(2/3)*max(stress);
% alp_ben=mean(alp_ref) %0.783(more ari reduces the mean alp)
% NF_ana=W0.*(4*(E-k)*(1+nu)*...
%     (b-1)/(E*(E+k*nu)*b*(b+1)).*...
%     Smax_ben.^(b+1).*(y-1/3.*lam.*m).^...
%     (1-b).*(1-alp_ben)).^-1%6.17e+03
% 
% error_iter_ana=(NF_num-NF_ana)/NF_num%-5.385e-02
