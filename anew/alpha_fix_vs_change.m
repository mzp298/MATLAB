...
W0=2.25e8;            %dissipated energy to failure per unit volume
...
hTitle = title('\alpha evolution with number of test points in EP01' ,'Fontsize' ,35);
set(hTitle, 'FontSize', 40, 'FontWeight' , 'bold');
hLegend=legend([alp_change,alp_fix],'\alpha=\alpha(s_{min}) varying with stress(n_F=95426)',...
 '\alpha_m=mean[\alpha(s_{min})] fixed in cyclic load(n_F=95562)','Location','Best');
...
% saveas(gcf,'F:\Git\Anew\figures\alpha_fix_vs_change.png');
