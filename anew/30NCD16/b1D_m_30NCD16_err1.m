...
hydromax=1/3*sum(stressben(n)+m(n) +0+0);
hydromin=1/3*sum(-stressben(n)+m(n) +0+0);
hydromean=1/2*sqrt(hydromax^2+hydromin^2);
devmax=[stressben(n)+m(n) 0 0 ;0 0  0 ;0 0 0 ]-hydromax*eye(3);
devmin=[-stressben(n)+m(n) 0 0 ;0 0 0 ;0 0 0 ]-hydromin*eye(3);
...
hLegend=legend([err_ben_m],...
    'Bending test on 30NCD16 with mean stress','location','bestoutside');
set(hLegend, 'FontSize', 18);
set(hLegend,'Box','on');
set(hLegend,'EdgeColor',[1 1 1]); %set the edge colour of the legend to white 
% Adjust font
