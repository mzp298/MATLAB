...
sminmin=yield_min.*Smaxbt(n).^-1;

hydromin=1/3.*(-stressben(n)+m(n));
yield_max=y-lam.*hydromin;
sminmax=yield_max.*Smaxbt(n).^-1;

yield_m=(yield_min+yield_max)/2;
alphamax=(1-a.*(sminmax-1).^-fb);
alphamin=(1-a.*(sminmin-1).^-fb);
alpbt_m(n)=(alphamax+alphamin)/2;
%--------use average smin to get NF--------
NFbt_num(n)=(1-alpbt_m(n)).^-1*W0*E*(E+k*nu)*b*(b+1)*((4*(E-k)*(1+nu)*(b-1)))^-1.*yield_m.^(b-1).*Smaxbt(n).^(-b-1);
NFbt_num_VM(n)=(1-alpbt_m(n)).^-1*W0*E*(E+k*nu)*b*(b+1)*((4*(E-k)*(1+nu)*(b-1)))^-1.*yield_m.^(b-1).*Smaxbt_VM(n).^(-b-1);
n=n+1;
end
alpbt_m
figure(1);
...
hold on;
bt_num=(1-alpbt_m).^-1*W0*E*(E+k*nu)*b*(b+1)*((4*(E-k)*(1+nu)*(b-1)))^-1.*yield_m.^(b-1).*Smaxbt.^(-b-1);
