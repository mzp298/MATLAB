...
WF=2.2e7;             %dissipated energy to failure per unit volume
...
y=405e6;           %macroscopic yield stress
siglim=0.3*y;          %fatigue limit leading to cycles to failure=2e6(n=4e6)
E=214e9;              %Young's modulus
k=6e8;                 %hardening parameter
nu=0.3;                     %poisson's ratio
sigu=720e6;             %ultimite stress
n0=1;                   %number of initial local defects
lam=0.1555;               %hydrostatic pressure sensitivity
gam=0.1;             %material parameter from Chaboche law(Wohler curve exponent)
Smaxth=X/2*y     %215.6MPa
stress11=zeros(1e7,1);
stress12=zeros(1e7,1);
...
stress11(n)=4*0.57*siglim+0.57*siglim*sin(n/2*pi);
stress12(n)=0;
hydro=1/3*sum(stress11(1)+0+0);
yield=y-lam*hydro; %macro yield strength considering mean stress effect
dev1=[stress11(1) stress12(1) 0;stress12(1) 0 0;0 0 0]-hydro*eye(3);
...
while D<1
    ...
    dev1=[stress11(n) stress12(n) 0;stress12(n) 0 0;0 0 0]-hydro*eye(3);
    dev11=dev1(1,1); dev12=dev1(1,2); dev13=dev1(1,3);
    dev21=dev1(2,1); dev22=dev1(2,2); dev23=dev1(2,3);
    dev31=dev1(3,1); dev32=dev1(3,2); dev33=dev1(3,3);
    
    stress11(n+1)=4*0.57*siglim+0.57*siglim*sin((n+1)/2*pi);
    stress12(n+1)=0;
    hydro=1/3*sum(stress11(n+1)+0+0);
    yield=y-lam*hydro; %macro yield strength considering mean stress effect
    devn=[stress11(n+1) stress12(n+1) 0;stress12(n+1) 0 0;0 0 0]-hydro*eye(3);
...
toc;
disp(['Number of cycles is ' num2str(n/4) ' ']);
disp(['Time to failure is ' num2str(n/4*0.1/3600) ' hours.']);
NF=(766109+773123)/2;
disp(['Error is ' sprintf('%2.2f%%', ((NF-n/4)/NF)*100) ' .']);
...
xlwrite('Clement_ER7.xlsx',siglim,1,'H1');
xlwrite('Clement_ER7.xlsx',n,1,'D3');
