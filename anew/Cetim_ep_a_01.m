...
[time,cycles,force,forcecommande,displacement]=textread('F:\Git\Cetim\ep_a_01\Acqui_CV.txt','%s%s%s%s%s','headerlines',5);
...
% %---------------------Plot stress-----------------------------
figure(1);
stress=plot((1:1000)*step,stress11(1:1000));
grid on;
grid minor;
set(gca ,'FontSize',25);
hXLabel = xlabel('t(s)' ,'Fontsize' ,25);
hTitle =title('Stress evlolution of Ep\_01 sinusoidal load' ,'Fontsize' ,25);
...
saveas(gcf,'F:\Git\Anew\figures\ep01_stress.png');
...
set(gca ,'FontSize',25);
hXLabel = xlabel('t(s)' ,'Fontsize' ,25);
hTitle =title('Damage evolution of Ep\_01 sinusoidal load' ,'Fontsize' ,25);
...
set(gcf, 'PaperUnits', 'points'); %[ {inches} | centimeters | normalized | points ]
set(gcf, 'PaperPosition', [0 0 1080 608]); %set(gcf,'PaperPosition',[left,bottom,width,height])
saveas(gcf,'F:\Git\Anew\figures\ep01_damage.png');
