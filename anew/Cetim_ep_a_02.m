...
[time,cycles,force,forcecommande,displacement]=textread('F:\Git\Cetim\ep_a_02\Acqui_CV.txt','%s%s%s%s%s','headerlines',5);
...
% %---------------------Plot stress-----------------------------
figure(1);
stress=plot((1:1000)*step,stress11(1:1000),'LineWidth', 1);
grid on;
grid minor;
set(gca ,'FontSize',25);
hXLabel = xlabel('t(s)' ,'Fontsize' ,25);
hTitle =title('Stress evlolution of Ep\_02 sinusoidal load' ,'Fontsize' ,25);
...
saveas(gcf,'F:\Git\Anew\figures\ep02_stress.png');
...
set(gca ,'FontSize',25);
hXLabel = xlabel('t(s)' ,'Fontsize' ,25);
hTitle =title('Damage evolution of Ep\_02 sinusoidal load' ,'Fontsize' ,25);
...
set(gcf, 'PaperUnits', 'points'); %[ {inches} | centimeters | normalized | points ]
set(gcf, 'PaperPosition', [0 0 1080 608]); %set(gcf,'PaperPosition',[left,bottom,width,height])
saveas(gcf,'F:\Git\Anew\figures\ep02_damage.png');
