...
W01=3.27e8;            %dissipated energy to failure per unit volume
W02=1.5e8;            %dissipated energy to failure per unit volume
...
alphamax2=(1-a2.*(smina-1).^-b2);
alphamin2=(1-a2.*(sminb-1).^-b2);
alpm1=(alphamax1+alphamin1)/2;
alpm2=(alphamax2+alphamin2)/2;

NF1=(1-alpm1).^-1*W01*E*(E+k*nu)*b1*(b1+1)*((4*(E-k)*(1+nu)*(b1-1)))^-1.*yieldm.^(b1-1).*Smax.^(-b1-1);
NF2=(1-alpm2).^-1*W02*E*(E+k*nu)*b2*(b2+1)*((4*(E-k)*(1+nu)*(b2-1)))^-1.*yieldm.^(b2-1).*Smax.^(-b2-1);
...
hLegend=legend([sn1,sn2], 'W_F=3.27e8','W_F=1.5e8','Location','best');
