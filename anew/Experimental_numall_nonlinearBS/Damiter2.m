...
Smaxtrialg=1/sqrt(2).*sqrt(sum(trialtensor.^2));

xi=sqrt(2).*abs(Smax(n+1)-Smax(n)).*(k*E/(E-k)+E/(1+nu)).^-1;

% Sbtrial=Smaxtrial-bs.*(1-gam.*xi);
Sbtrialg=Smaxtrialg-bs;
eta=bsxfun(@minus,bsxfun(@times,Sbtrialg/yield(1),s),1); 
eta(eta<0)=0; %elastic regime goes to zero

bs=bs+(k*E/(E-k)*xi.*sign(hydro(n+1)-hydro(n))-gam.*xi.*bs).*sign(eta); %sign(eta)=1 means plasticity regime
%if the trial11>(1+eta) at certain scale, return to yield stress, if not,
%keep trial11, expressed as:
...
Ws=(bsxfun(@minus,Sbtrialg,bsxfun(@rdivide, yield(n+1),s))<=0).*...
        (0)+...
        (bsxfun(@minus,Sbtrialg,bsxfun(@rdivide, yield(n+1),s))>0).*...
        ((E-k)*(1+nu)*(2*E*(E+k*nu))^-1*...
        bsxfun(@times,weight,bsxfun(@rdivide,bsxfun(@times,bsxfun(@minus,Sbtrialg,bsxfun(@rdivide, yield(n+1),s)),yield(n+1)),s)));
W= sum(Ws);
