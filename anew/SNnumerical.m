...
alphamax1=(1-a1.*(smina-1).^-b1);
alphamin1=(1-a1.*(sminb-1).^-b1);
...
grid minor;
xlim([1e3 2e7]);
set(gca,'XTick',[1e0,1e1,1e2,1e3,1e4,1e5,1e6,1e7,1e8,1e9],'FontSize',35 );
...
%  for load = [2.2e8 1.8e8  1.5e8 1.3e8 1e8  0.7e8 0.5e8 0.4e8]
% %---------------------3 Numerical method-----------------------------
% D=1e-16;
% n=1;       %initial recording point
% %---------------------to get the the first Sb-----------------------------
% loadtensor= [load 0 0;0 0 0;0 0 0];
% stress11=m+load*cosd(n*360/stepnumber);
...
%     dev31=dev1(3,1); dev32=dev1(3,2); dev33=dev1(3,3);
%     
%     stress11=m+load*cosd((n+1)*360/stepnumber);
%     hydro=1/3*sum(stress11+0+0);
...
% NF=n/stepnumber
% hold on;
% plot(NF,load, 'o','MarkerSize',20, 'MarkerFaceColor','k', 'MarkerEdgeColor',[1 0.5 0]);
% end
hold on
load = [2.2e8 1.8e8  1.5e8 1.3e8 1e8  0.7e8 0.5e8 0.4e8]
NF =[7.9465e3  2.70545e4 6.38255e4  114426  2.884425e5  8.320605e5 2024415 3495793]
numerical=plot(NF,load, 'o','MarkerSize',20, 'MarkerFaceColor','k', 'MarkerEdgeColor',[1 0.5 0]);
hLegend=legend([sn1,numerical], 'Analytica S-N curve with \alpha_m','Calculated numerical points','Location','northeast');
set([hLegend, gca], 'FontSize', 30)
set(hLegend,'Box','on');
set(hLegend,'EdgeColor',[1 1 1]); %set the edge colour of the legend to white 
 saveas(gcf,'F:\Git\Anew\figures\SNnumerical.png');