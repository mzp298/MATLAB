...
lam2=0.5;               %hydrostatic pressure sensitivity
...
yielda1=y-lam1.*hydroa;
smina1=yielda1.*Smax.^-1;
hydrob=1/3.*(-stress11+m);
yieldb1=y-lam1.*hydrob;
sminb1=yieldb1.*Smax.^-1;
yieldm1=(yielda1+yieldb1)/2;
alphamax1=(1-a1.*(smina1-1).^-b1);
alphamin1=(1-a1.*(sminb1-1).^-b1);
alpm1=(alphamax1+alphamin1)/2;

yielda2=y-lam2.*hydroa;
smina2=yielda2.*Smax.^-1;
yieldb2=y-lam2.*hydrob;
sminb2=yieldb2.*Smax.^-1;
yieldm2=(yielda2+yieldb2)/2;
alphamax2=(1-a2.*(smina2-1).^-b2);
alphamin2=(1-a2.*(sminb2-1).^-b2);
alpm2=(alphamax2+alphamin2)/2;

NF1=(1-alpm1).^-1*W0*E*(E+k*nu)*b1*(b1+1)*((4*(E-k)*(1+nu)*(b1-1)))^-1.*yieldm1.^(b1-1).*Smax.^(-b1-1);
NF2=(1-alpm2).^-1*W0*E*(E+k*nu)*b2*(b2+1)*((4*(E-k)*(1+nu)*(b2-1)))^-1.*yieldm2.^(b2-1).*Smax.^(-b2-1);
...
% hTitle =title('S-N curve at constant amplitude stress with R=-1 in our model with different $\lambda$');
hXLabel = xlabel('N_F');
hYLabel =ylabel('Stress');

hLegend=legend([sn1,sn2], '\lambda=0.1','\lambda=0.5','Location','northeast');
...
set(gcf, 'PaperPosition', [0 0 1200 1000]); %set(gcf,'PaperPosition',[left,bottom,width,height])
 saveas(gcf,'F:\Git\Anew\figures\SNlam.png');