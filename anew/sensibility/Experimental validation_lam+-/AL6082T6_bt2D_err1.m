...
for  i=1:length(NF)
    ...
    Smax_bt2d(i)=(max(Smax)-min(Smax))/2; %sqrt(2/3).*stressben
    alp_bt2d(i)=mean(alp);
...
fun_analytical=@(parameters)parameters(1).*E.*(E+k.*nu).*parameters(2).*(parameters(2)+1).*...
    ...
    (Smax_bt2d.^(parameters(2)+1).*(y-1/3.*lamplus.*hydroplus).^(1-parameters(2))...
    +Smax_bt2d.^(parameters(2)+1).*(y-1/3.*lamminus.*hydrominus).^(1-parameters(2))).^-1-NF';
