...
if hydro(n+1)>=0
yield(n+1)=y-lamplus*hydro(n+1); %yield stress at time step n+1
else
yield(n+1)=y-lamminus*hydro(n+1);    
end
devn=tensor-hydro(n+1)*eye(3);
...
g=1; %first reference index when creating
    if abs(alp(n+1)-alp_ref(g))>delta_alp %----giving scalar value to iteration after the addaptation cycle(decrease time step)
        alp_ref(g+1)=alp(n);
        n_ref(g+1)=n;
        W_ref(g+1)=W_accumulate(n_ref(g+1)+1)-W_accumulate(n_ref(g)+1);
    g=g+1;
    end
