...
b=b_test(1);
run('SM45C_bt1D_new_alp.m'); %-to get new alp(1) ---
...
run('SM45C_bt1D_new_alp.m'); %--to get new alp(p) using b_test(p)---
parameters0=[W0(p),b_test(p)];  %from pth fit
[parameters_fit,resnorm,exitflag]=lsqnonlin(fun_analytical,parameters0,lb,ub);
p=p+1;
W0(p)=parameters_fit(1);  %to get new fit
b_test(p)=parameters_fit(2)
if abs(b_test(p)-b_test(2))/b_test(2)>20 %if new fit is too far from original fit
    b_test(p)=b_test(2);
    W0(p)=W0(2);
else
    b_test(p)=b_test(p);
...
save('SM45C.mat','W0','b','NFben_num','NFtor_num','alp_ben','alp_tor','a','E','k','nu','y','stepnumber','cycles','delta_alp','cycles90'); %save the best fit W0, b, alp_ben, alp_tor