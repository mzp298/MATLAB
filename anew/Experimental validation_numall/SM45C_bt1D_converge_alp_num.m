...
fun_analytical=@(parameters)parameters(1).*E.*(E+k.*nu).*parameters(2).*(parameters(2)+1).*...
    ...
    +[Smax_tor].^(parameters(2)+1).*(y-parameters(4).*[zeros(size(stresstor))]').^(1-parameters(2))).^-1-[0.7.*NFtor]';
% %-------------lsqnonlin locally stuck, issue new parameters of the non-continuous function using random
% %-------------generated new values-------------
parameters=[3e8,6,0,0];
lb  =  [1e5,   1,0,0];
ub =  [1e12,  8,5,5];    %bmax=1 gives 7.26e+06
...
NFtor_num=fun_analytical(parameters)+[0.7.*NFtor]';
...
save('SM45C.mat','NFben_num','NFtor_num','alp_ben','alp_tor','lamplus','lamminus','-append');
...
%         e=e+1;
%     end
%     NFtor_num2(i)=e/stepnumber
%  end
% save('SM45C.mat','NFtor_num2','-append');
