...
M=(1-0.3*hydro/sigu); %M function in Chaboche model
M(M<0)=0;
yield(1)=y*M;  %macro yield strength considering mean stress effect
...
while G<1
    ...
    M=(1-0.3*hydro/sigu); %M function in Chaboche model
    M(M<0)=0;
    yield(n+1)=y*M; %macro yield strength considering mean stress effect
...
figure(1);
 hold on;
  ...
  Trial61=plot ((1:n)*step,normtrial(1:n,61),'LineStyle', 'none','LineWidth', 1,'Marker', 's', 'MarkerSize',12, ...
   ...
   Sb1=plot ((1:n)*step,normSb(1:n,1),'LineStyle', 'none','LineWidth', 1,'Marker', '^', 'MarkerSize', 6, ...
    'MarkerEdgeColor','none', 'MarkerFaceColor',[255 222 173]/255);
   Sb61=plot ((1:n)*step,normSb(1:n,61),'LineStyle', 'none','LineWidth', 1,'Marker', 's', 'MarkerSize',6, ...
    'MarkerEdgeColor','none', 'MarkerFaceColor',[156 156 156]/255);
  yield1=plot ((1:n)*step,yield(1:n)*s(1).^-1,'LineStyle', 'none','LineWidth', 1,'Marker', 'o', 'MarkerSize', 6, ...
    'MarkerEdgeColor', 'none', 'MarkerFaceColor','b');
  yield61=plot ((1:n)*step,yield(1:n)*s(61).^-1, 'LineStyle', 'none','LineWidth', 1, 'Marker', 'o', 'MarkerSize', 6, ...
    'MarkerEdgeColor',  'none', 'MarkerFaceColor' , 'c');
