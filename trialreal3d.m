...
stress31=stress13;
stress32=stress23;
stress33=1/A*(cx*forcelx*sin(thetax)^2*sin(phix)^2+cy*forcely*sin(thetay)^2*sin(phiy)^2);% [max(stress11) max(stress12) max(stress13);
...
yield(1)=y-lam*m(1);
yield(yield<0)=0;
dev1=[stress11(1) stress12(1) stress13(1);stress21(1) stress22(1) stress23(1);stress31(1) stress32(1) stress33(1)]-m(1)*diag([1,1,1]);
...
for i=1:25 %creat a set of Ws[n*25 vector]
   ...
   G = G+W(1)/WF; %1.322163316411401e-03
  
while G<0.1
   m(n+1)=1/3*sum(stress11(n+1)+stress22(n+1)+stress33(n+1));
   yield(n+1)=y-lam*m(n+1);
   yield(yield<0)=0;
   devn=[stress11(n+1) stress12(n+1) stress13(n+1);stress21(n+1) stress22(n+1) stress23(n+1);stress31(n+1) stress32(n+1) stress33(n+1)]...
       -m(n+1)*diag([1,1,1]);
  ...
  G = G+W(n+1)/WF; %2nd step=1.596288815802358e-03
